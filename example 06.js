...
var board = new firmata.Board("/dev/ttyACM0", function(){ // ACM Abstract Control Model for serial communication with Arduino (could be USB)
    ...
    console.log("Activation of Pin 8");
    board.pinMode(8, board.MODES.OUTPUT);console.log("Activation of Pin 8");
    board.pinMode(8, board.MODES.OUTPUT);
});

function handler(req, res){
    fs.readFile(__dirname + "/example 06.html",
    function(err, data){
        if(err){
...
io.sockets.on("connection", function(socket) {
    socket.on("commandToArduino", function(commandNo){
        ...
        if (commandNo == "7") {
            board.digitalWrite(10, board.LOW); // write LOW on pin 8
        }
        if (commandNo == "8") {
